...
class LoadTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        div = torch.exp(torch.arange(0, self.input_projection.out_features, 2, device=x.device) *
                        (-math.log(10000.0) / self.input_projection.out_features))
        encoding = torch.zeros(length, self.input_projection.out_features, device=x.device, dtype=x.dtype)
        encoding[:, 0::2] = torch.sin(position[:, None] * div)
        encoding[:, 1::2] = torch.cos(position[:, None] * div[:encoding[:, 1::2].shape[1]])
